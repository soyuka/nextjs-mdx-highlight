...
export default function SectionGuide({ children }: PropsWithChildren<any>) {
  ...
  Children.map(children, (child) => {
    ...
    if (type === "a" && anyProps?.id?.includes("section")) {
      id = anyProps?.id;
      return null;
    }
    if (type === "div" && !anyProps?.children?.length) return null;
    if (anyProps["data-rehype-pretty-code-fragment"] !== undefined) {
      right.push(child);
