...
export default function CodeSelector({ children }: PropsWithChildren<any>) {
  ...
  const types = childs?.map((item) => item.props["data-language"]).filter((value, index, array) => array.indexOf(value) === index);
